...
function bootPage(opts){
  ...
  auth.onAuthStateChanged(async user=>{
    ...
    try{
      ...
      /* Keep password-recovery enforcement live. If Minekid123 issues a reset
         while an old session is still open, that session is redirected as soon
         as the profile update reaches Firestore. */
      let lastMustChange=G.actorProfile.mustChangePassword===true;
      const profileGuard=uref.onSnapshot(ds=>{
        if(!ds.exists)return;const next=ds.data()||{},must=next.mustChangePassword===true;
        if(must&&!lastMustChange){location.replace('index.html?changePassword=1');return;}
        lastMustChange=must;
      },e=>console.warn('password profile guard',e));
      G.unsubs.push(profileGuard);
      ...
      /* Temporary passwords must be replaced from the secure portal before any
         member or administrative application can be opened. */
      const onPortal = opts.requireAuth === false || /(?:^|\/)index\.html$/i.test(location.pathname) || location.pathname.endsWith('/');
      if(G.actorProfile.mustChangePassword === true && !onPortal){
        location.replace('index.html?changePassword=1'); return;
      }
      G.actorRole='user';
