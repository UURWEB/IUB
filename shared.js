...
async function setPresence(online, state){
  ...
  try{
    await db.collection('presence').doc(G.user.uid).set({
      username:G.username,
      online:!!online,
      state:resolvedState,
      page:document.body.dataset.brand||'portal',
...
function bootPage(opts){
  ...
  auth.onAuthStateChanged(async user=>{
    ...
    try{
      ...
      G.profile=snap.data()||{};
      if(String(G.profile.loginStatus||'active')==='retired'){
...
function mountSwitcher(current){
  ...
  wrap.innerHTML=`<div class="uur-switcher-menu">
      <a href="index.html"><span style="width:30px;text-align:center;font-size:16px">⌂</span><span>Union Portal Home</span></a>${servicesLink}${adminLink}${advancedLink}${links}
